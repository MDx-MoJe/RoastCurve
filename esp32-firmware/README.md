...
固件所有引脚都是 `roast_bridge.ino` 顶部的常量，改数字即可换脚：

```cpp
constexpr int PIN_485_TX  = 17;   // RS485 发送 → 模块 DI
constexpr int PIN_485_RX  = 18;   // RS485 接收 ← 模块 RO
constexpr int PIN_FAN_PWM = 2;    // 风扇 PWM 输出（v1.3）
```

ESP32-S3 内部有 GPIO 矩阵（任意外设信号可路由到几乎所有引脚），
17/18/2 没有任何特殊之处，改成 14/15 等完全可行。
换脚三步：改常量 → 重新编译烧录 → 按新脚接线，三者同步。
...
| 26~32 | 片内 SPI 闪存 / PSRAM | 变砖（N16R8 板必碰不得） |
| 0 | BOOT 键 | 按住进烧录模式检测失效 |
| 3/45/46 | strapping（启动配置）脚 | 上电电压/启动模式异常 |
| 43/44 | UART0 默认控制台 | 串口日志丢失 |
| 48 | 板载 WS2812 状态灯 | 指示灯失效 |

其余 GPIO 基本自由。波特率 1200 与引脚无关（那是自动收发模块的信号质量结论），换脚不用改。
