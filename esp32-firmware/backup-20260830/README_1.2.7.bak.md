...
```
ESP32-S3 DevKitC-1          TTL转RS485模块（带 DIR 或自动收发款）
┌──────────────┐            ┌──────────┐
│  GPIO18 (TX) ├────────────┤ TXD      │
│  GPIO17 (RX) ├────────────┤ RXD      │
│  GPIO21      ├────────────┤ DIR      │  ← 仅带 DIR 脚的模块需要；自动收发模块悬空
│  3V3         ├────────────┤ VCC      │
│  GND         ├────────────┤ GND      │
└──────────────┘            └──┬───┬───┘
                               A   B
...
⚠️ **线序铁律（2026-08-30 实测定稿）**：这类模块的丝印是「接设备视角」——
**TXD 是模块输入、RXD 是输出**。固件 TX（GPIO18）必须接模块 **TXD**，
固件 RX（GPIO17）接模块 **RXD**。按「设备视角」接（18→RXD、17→TXD）
会全线死寂（字节永远上不了总线），这是「读通写不通/全哑」的头号真凶。

自动收发模块无 DIR 脚，GPIO21 悬空即可（固件 DIR 控制空转无副作用）。
