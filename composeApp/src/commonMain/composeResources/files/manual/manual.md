...
| 红色常亮 | ⚠ 安全模式生效 | SV 已压至 70°C；App 会弹窗让你选保持或下调 |
...
1. **触发条件是"无主控且炉子在加热"**：主控失联且当前 SV ≥ 安全温度（正在烘焙/加热）才触发；待机（SV 已低于安全温度）失联不触发——不会把待机炉子反向加热到保温值空烧
2. **30 秒倒计时**：所有旁观者弹横幅倒计秒数 + 大按钮「立即接管」
3. **接管即取消倒计时**：任何人接管，倒计时终止，豆子救回
4. **倒计时归零 → 安全模式一段**：SV 写 70°C（可配置 60~80），风机 30%。70°C 的含义：保温不熄火——不烤坏豆子、不失控升温、能耗极低，给短断线留抢救窗口
5. **粘性 + App 交互**：安全模式下主控重连不会自动恢复原 SV。App 检测到保温会弹窗由你决定：**继续保持**（短断线抢救场景）或**立即下调到待机温度**（确认无需保温）。为什么粘性：如果断线后重连瞬间把 SV 拉回高温，锅炉和豆子都遭殃
