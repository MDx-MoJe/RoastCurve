...
本次发布围绕「烘焙自主化」做了系统性升级，强烈建议同时更新 App 与固件：

### 跟随曲线：从「实时受控」改为「自主执行」

- **移除脱轨熔断**：旧逻辑下豆温与目标差 >15°C 持续 30s 会中断跟随，空锅 / 大风散热快时容易误触发。现在曲线自主跑完，不再中途夭折
- **走完自动回落**：曲线结束后 SV 自动降到收尾值并调低风机（默认 25°C / 25%），不再停在终点干烧
- **App 回落值可调**：设置 →「跟随结束回落」，温度 20~120°C、风机 0~100% 自由调
- **保留安全网**：实测温度 ≥250°C 持续 5s 才硬中断回落（防探头损坏/加热失控带飞炉子），正常烘焙不会触及

### Web UI（浏览器控制台）：全面重构

- **浏览器直接烘焙**：无需 App，连板子 WiFi 后打开网页即可监控/控制/记录
- **记录不丢**：炉次数据由固件自主采样（每 5s 一点，最长 120 分钟），网页刷新 / 锁屏 / 换设备都不丢曲线——刷新后自动恢复计时、事件与曲线
- **手绘豆温曲线**：🖌 绘制曲线，点加锚点、拖动调点、实时看每个锚点的温度/时间，存为模板直接跟随
- **模板生态**：本炉存模板、Artisan .alog 导入、模板管理，与 App 模板体系互通
- **风机实时控制**：滑块即生效 + 回传确认反馈
- **导出 zip**：全部/单条记录导出标准 zip（内含 JSON），兼容导入；不再裸 JSON
- **信号强度显示** + 曲线事件标记点

### GPIO 自定义（DIY 换板不换线）

- **RS485 TX/RX + 风机 PWM 三脚可配**（App 设置 → 桥接器 GPIO 引脚，或 Web UI 设置 → GPIO 引脚配置），改完重启生效
- 换 ESP32 板子不用再改代码重刷，网页里选好引脚即可
- 可用池已排除硬件禁区（flash 26-32 / PSRAM 33-37 / USB 19-20 / 争议脚 1,3,45,46），三脚互斥分配
- 改错失联？长按 BOOT 键 3 秒恢复默认（17/18/2）

### 安全修复

- **锁屏/断连不再死锁**：跟随中主控断开后，新页面可正常「接管跟随」/「停止跟随」（之前会卡死只能急停）
- **看门狗倒计时溢出修复**：不再出现 42 亿秒的虚假倒计时

### ⚠️ 注意事项

- **固件与 App 建议一起更新**：新跟随语义（自主执行+回落）两端需要配套
- **4MB flash 板子已不支持**（v1.4 起），请用 8MB 或 16MB 板；8MB 板编译时用 `default_8MB` 分区（详见固件目录 PARTITIONS.md）
- **跟随中锁屏/断线 = 炉子继续按曲线跑**（这是特性）：无人看管时曲线会跑完并自动回落到 25°C，回来接管即可
- Web UI 与 App 是**两套独立跟随入口**，同一时间只用一端启动跟随
- **GPIO 引脚配置需要固件 v1.8.1+**：旧固件（≤v1.4 无版本号 / v1.5-1.7）在 App 或网页里会看到「固件版本过低」提示，需 USB 刷 v1.8.3
