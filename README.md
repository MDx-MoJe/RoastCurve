...
**App v1.3.27**（versionCode 165）· **桥接固件 v1.9.4**

建议同时更新 App 与固件：

### 安全看门狗 v2（固件 + App 联动）

- **待机不误保温**：主控失联仅在炉子加热中（SV ≥ 安全温度）触发保温；待机失联保持原温度，不再空烧
- **保温后 App 弹窗决策**：安全模式激活时 App 提示「继续保持」或「立即下调到待机温度」，由你决定
- **跟随中失联不受看门狗打扰**：曲线自主跑完，回来接管即可
...
**怎么用**：板子配网后（连热点 `RoastBridge-Setup` 密码 `roast1234` → 开 `http://192.168.4.1` 填 WiFi），手机/电脑连同一 WiFi，浏览器打开 `http://<板子IP>:8898` 即可。点「接管控制」→「入豆」开始记录，选模板或手绘曲线点「启动跟随」自动烘焙。详细步骤见《[快速上手](docs/快速上手.md)》。

**iOS 用户**：烤豆 App 的 iOS 版开发中，**推荐直接用 Web UI**——iPhone/iPad 浏览器打开上面的地址即可完整烘焙（配网、监控、跟随、记录全支持）。
...
- **[《快速上手》](docs/快速上手.md)**：新手从零到第一炉（下载 / 接线 / 刷固件 / 配网 / Web UI / App）
- **[《完整用户指导手册》](docs/完整用户指导手册.md)**：全链路详解（架构 / 硬件 / Web UI / App / 跟随 / 安全 / 排障）
